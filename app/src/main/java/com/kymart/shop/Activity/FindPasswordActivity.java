...
public class FindPasswordActivity extends BaseActivityother {
    ...
    protected void initData() {
        UUID = InstalltionId.id(FindPasswordActivity.this);
    ...
    void requestImgCode(final String uuid){//请求图片验证码
        new Volley_Utils(new Interface_volley_respose() {
            ...
            public void onError(int error) {
                ToastUtils.showToast(FindPasswordActivity.this,"请输入正确的手机号");
    ...
    void requestMessageCode(Map map){//发送短信验证码
        ...
        }).postHttp(BaseUrl.BaseURL+BaseUrl.messageCode_find,this,1,map);
    }
    void requestRegister(Map map){//提交按钮点击
        new  Volley_Utils(new Interface_volley_respose() {
            @Override
            public void onSuccesses(String respose) {
                int  status=0;
